 ...
 * TODO
 *
 * - Code to expose safe areas of digging should examine neighbours of area to clear,
 *   and only clear area if there is no neighbour mine. More like original minesweeper.
 * - Blink exposed areas as a hint, 1 blink = 1 neighbour.  Eye strain mode!
 */
...
byte letters[53][8]; // Some other stuff we don't use.
byte numbers[10][2][8]; // More stuff we don't use.
byte n1[8],n1b[8],n2[8],n2b[8],n3[8],n3b[8],n4[8],n4b[8],n5[8],n5b[8],n6[8],n6b[8],n7[8],n7b[8],n8[8],n8b[8],n9[8],n9b[8],n0[8],n0b[8]; // Even more stuff we don't use.
...
void loop() {
  ...
  if (!editMode) { // Move cursor
    ...
    if (gamer.isPressed(UP)) { // Toggle!
      if(mineMap[currentX][currentY] != 2) {
    ...
    if (gamer.isPressed(RIGHT) && mineMap[currentX][currentY] == 3) {
      ...
      Serial.println(dig2);
      for(int p=0;p<8;p++) {
        result[p]=numbers[0][0][p]^numbers[dig2][1][p];
      }
  ...
  if (!boom) {
    for(int x=0; x<8;x++) {
      for(int y=0;y<8;y++) {
        ...
        } else if (mineMap[x][y] == 3) { // Clear, unset.
...
void printString(String string) {
  ...
  while(printing) {
    ...
    for(int x=0;x<1;x++) {  //this is only here because I'm too lazy to remove it
